library(ggplot2)
tot <-read.csv("south America.csv",header=TRUE)
#x<-data.frame(tot)
#print(x)
plot("tot",type="b",lwd=2,
     xaxt="n",ylim=c(15,450),col="blue",
     xlab="YEAR",ylab="Mortality Rate",
     main="Moratality rate in %")
axis(1,at=1:length(YEAR),label=YEAR)
...
legend("bottomleft",legend=c("Argentina","Bolivia","Brazil","Chile","Colombia","Ecador","Guyana","Paraguay","Peru","Suriname","Uruguay","Venezuela"),
       lty=1,lwd=2,pch=21,col=c("blue","red","green","purple","black"),
       ncol=2,bty="n",cex=0.8,
       text.col=c("blue","red","green","purple","black","violet","grey","orange","Pink","cyan","brown","yellow"),
       inset=0.01)
png(file="ENC222_0131-2018.png")
dev.off()